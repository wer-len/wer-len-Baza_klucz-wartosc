...
class Store:
    ...
    def __init__(self):
        self._store = {"__default__":{}} #baza danych z domyslnym namespacem default
        self._currentNamespace = None         #plik zawsze tam trafia jeżeli nie podamy namespace
    ...
    def put(self, key, value, *, namespace=None, guard=None):
        ...
        if key in self._store[namespace]:
            #sprawdzanie guarda
            v = self._store[namespace][key] #wyciagam to co jest pod tym kluczem
            if v["guard"] == guard:
    ...
    def get(self, key, *, namespace=None):
        namespace = self._checkNamespace(namespace)  # funkcja sprawdza czy uzyjemy dobrej nazwy
        if namespace == None:
            return MESSAGES.INCORRECT_NAMESPACE
